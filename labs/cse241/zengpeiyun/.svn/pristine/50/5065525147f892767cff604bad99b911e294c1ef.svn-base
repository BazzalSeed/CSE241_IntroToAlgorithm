...
public class Sort {
    ...
    public static void helpmsort(XYPoint[] A, int indexStart, int indexFinish, Comparator lessThan){
        if(indexStart < indexFinish){
            int indexMiddle = (int)Math.round((indexFinish + indexStart)/2);
            helpmsort(A, indexStart, indexMiddle, lessThan);
            helpmsort(A, indexMiddle + 1, indexFinish, lessThan);
            merge(A, indexStart, indexMiddle, indexFinish, lessThan);
        }
    }
    
    public static void merge(XYPoint[] A, int indexStart, int indexMiddle, int indexFinish, Comparator lessThan){
        // Define Split Sizes
        int leftSize = indexMiddle - indexStart + 1;
        int rightSize = indexFinish - indexMiddle;
